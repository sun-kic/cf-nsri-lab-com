...
class TodayController extends Controller
{
    ...
    private const DEFAULT_POWER_RATE = 0.445;
    //
    ...
    public function route(Request $request)
    {
        ...
        if (!is_array($jsonData) || Arr::get($jsonData, 'status') !== 'OK') {
            return 0;
        }

        return Arr::get($jsonData, 'routes.0.legs.0.distance.value', 0) / 1000;
    }

    public function store(Request $request) {

        $formFields = $this->normaliseFormFields($request);

        $rates = $this->rates();
        $powerRates = $rates['power_company'];
        $foodCarbonRate = $rates['food'];
        $riceCarbonRate = $rates['rice'];
        $vegetableVolumeRate = $rates['vegetable_volume'];
        $vegetableCarbonRate = $rates['vegetable_carbon'];
        $mainCarbonRate = $rates['main'];
        $mainVolumeRate = $rates['main_volume'];
        $sportsCarbonRate = $rates['sports_store'];
        $restCarbonRate = $rates['rest'];
        $moveTypeRate = $rates['move'];

        //計算用の諸データ
        $work_office_time = $this->countSelections($formFields['work_office'] ?? null);
        $work_soho_time = $this->countSelections($formFields['work_soho'] ?? null);
        $work_3pl_time = $this->countSelections($formFields['work_3pl'] ?? null);

        $works_time = $work_office_time + $work_soho_time + $work_3pl_time; //仕事時間の合計

        $life_time = $this->countSelections($formFields['life'] ?? null); //生活時間の合計

        $move_time = $this->countSelections($formFields['move'] ?? null); //移動時間の合計
        ...
        $defaultPowerRate = $powerRates['その他'] ?? self::DEFAULT_POWER_RATE;
        $user_power_rate = $powerRates[$power_company] ?? $defaultPowerRate;

        //食事計算用データ

        $shopping_carbon_rate = $rates['shopping'];
        ...
        if (array_key_exists("sports_place",$formFields) and !is_null($formFields["sports_type"]) and !is_null($formFields["sports_time"])){
            if ($formFields["sports_place"] == "屋外"){
                ...
                $sports_carbon = ($sportsCarbonRate[$formFields["sports_type"]] ?? 0) * $formFields["sports_time"]; 
            }
        }else{
            $sports_carbon = 0;
        }
        ...
        if (!is_null($formFields["rest_type"]) and !is_null($formFields["rest_time"])){
            $rest_carbon = ($restCarbonRate[$formFields["rest_type"]] ?? 0) *0.445 * $formFields["rest_time"]; 
        }else{
            $rest_carbon = 0;
        }
        ...
        if (array_key_exists("breakfast_type",$formFields) and array_key_exists("breakfast_vegetable_produced",$formFields) and array_key_exists("breakfast_volumn",$formFields)){
            $breakfast_rice_carbon = $foodCarbonRate[$formFields["breakfast_type"]][$formFields["breakfast_volumn"]] * $riceCarbonRate[$formFields["breakfast_type"]]/1000;
            ...
            if (
                ...
                if ($formFields["breakfast_vegetable_produced"] == "地元")
                    $breakfast_vegetable_carbon = $vegetableCarbonRate[$formFields["breakfast_vegetable_type"]][$season] * $vegetableVolumeRate[$formFields["breakfast_type"]][$formFields["breakfast_vegetable_volumn"]]/1000*0.9;
                else{
                    $breakfast_vegetable_carbon = $vegetableCarbonRate[$formFields["breakfast_vegetable_type"]][$season] * $vegetableVolumeRate[$formFields["breakfast_type"]][$formFields["breakfast_vegetable_volumn"]]/1000;
                }
            }else{
            ...
            if (
                ...
                if ($formFields["breakfast_main_produced"] == "地元"){
                    $breakfast_main_carbon = $mainCarbonRate[$formFields["breakfast_main_type"]]*$mainVolumeRate[$formFields["breakfast_type"]][$formFields["breakfast_main_volumn"]]/1000*0.9;
                }else{
                    $breakfast_main_carbon = $mainCarbonRate[$formFields["breakfast_main_type"]]*$mainVolumeRate[$formFields["breakfast_type"]][$formFields["breakfast_main_volumn"]]/1000;
                }
            }else{
        ...
        if (array_key_exists("lunch_type",$formFields) and array_key_exists("lunch_vegetable_produced",$formFields) and array_key_exists("lunch_volumn",$formFields)){
            $lunch_rice_carbon = $foodCarbonRate[$formFields["lunch_type"]][$formFields["lunch_volumn"]] * $riceCarbonRate[$formFields["lunch_type"]]/1000;
            ...
            if (
                ...
                if ($formFields["lunch_vegetable_produced"] == "地元")
                    $lunch_vegetable_carbon = $vegetableCarbonRate[$formFields["lunch_vegetable_type"]][$season] * $vegetableVolumeRate[$formFields["lunch_type"]][$formFields["lunch_vegetable_volumn"]]/1000*0.9;
                else{
                    $lunch_vegetable_carbon = $vegetableCarbonRate[$formFields["lunch_vegetable_type"]][$season] * $vegetableVolumeRate[$formFields["lunch_type"]][$formFields["lunch_vegetable_volumn"]]/1000;
                }
            }else{
            ...
            if (
                ...
                if ($formFields["lunch_main_produced"] == "地元"){
                    $lunch_main_carbon = $mainCarbonRate[$formFields["lunch_main_type"]]*$mainVolumeRate[$formFields["lunch_type"]][$formFields["lunch_main_volumn"]]/1000*0.9;
                }else{
                    $lunch_main_carbon = $mainCarbonRate[$formFields["lunch_main_type"]]*$mainVolumeRate[$formFields["lunch_type"]][$formFields["lunch_main_volumn"]]/1000;
                }
            }else{
                $lunch_main_carbon = 0;
            }
        ...
        if (array_key_exists("dinner_type",$formFields) and array_key_exists("dinner_vegetable_produced",$formFields) and array_key_exists("dinner_volumn",$formFields)){
            $dinner_rice_carbon = $foodCarbonRate[$formFields["dinner_type"]][$formFields["dinner_volumn"]] * $riceCarbonRate[$formFields["dinner_type"]]/1000;
            ...
            if (
                ...
                if ($formFields["dinner_vegetable_produced"] == "地元")
                    $dinner_vegetable_carbon = $vegetableCarbonRate[$formFields["dinner_vegetable_type"]][$season] * $vegetableVolumeRate[$formFields["dinner_type"]][$formFields["dinner_vegetable_volumn"]]/1000*0.9;
                else{
                    $dinner_vegetable_carbon = $vegetableCarbonRate[$formFields["dinner_vegetable_type"]][$season] * $vegetableVolumeRate[$formFields["dinner_type"]][$formFields["dinner_vegetable_volumn"]]/1000;
                }
            }else{
            ...
            if (
                ...
                if ($formFields["dinner_main_produced"] == "地元"){
                    $dinner_main_carbon = $mainCarbonRate[$formFields["dinner_main_type"]]*$mainVolumeRate[$formFields["dinner_type"]][$formFields["dinner_main_volumn"]]/1000*0.9;
                }else{
                    $dinner_main_carbon = $mainCarbonRate[$formFields["dinner_main_type"]]*$mainVolumeRate[$formFields["dinner_type"]][$formFields["dinner_main_volumn"]]/1000;
                }
            }else{
        ...
        if (!is_null($formFields["move_out_departure1"]) or !is_null($formFields["move_out_arrival1"]) or !is_null($formFields["move_out_type1"]) or !is_null($formFields["move_out_distance1"])){
            ...
            $move_out1_carbon = $distance1 * ($moveTypeRate[$formFields["move_out_type1"]] ?? 0);
        }else{
            $move_out1_carbon = 0;
        }
        ...
        if (!is_null($formFields["move_out_departure2"]) or !is_null($formFields["move_out_arrival2"]) or !is_null($formFields["move_out_type2"]) or !is_null($formFields["move_out_distance2"])){
            ...
            $move_out2_carbon = $distance2 * ($moveTypeRate[$formFields["move_out_type2"]] ?? 0);
        }else{
            $move_out2_carbon = 0;
        }
        ...
        if (!is_null($formFields["move_out_departure3"]) or !is_null($formFields["move_out_arrival3"]) or !is_null($formFields["move_out_type3"]) or !is_null($formFields["move_out_distance3"])){
            ...
            $move_out3_carbon = $distance3 * ($moveTypeRate[$formFields["move_out_type3"]] ?? 0);
        }else{
            $move_out3_carbon = 0;
        }
        ...
        if (!is_null($formFields["move_out_departure4"]) or !is_null($formFields["move_out_arrival4"]) or !is_null($formFields["move_out_type4"]) or !is_null($formFields["move_out_distance4"])){
            ...
            $move_out4_carbon = $distance4 * ($moveTypeRate[$formFields["move_out_type4"]] ?? 0);
        }else{
            $move_out4_carbon = 0;
        }
    ...
    public function update(Request $request, Today $today) {
        ...
        $mainCarbonRate = $rates['main'];
        $mainVolumeRate = $rates['main_volume'];
        $sportsCarbonRate = $rates['sports_update'];
        $restCarbonRate = $rates['rest'];
        $moveTypeRate = $rates['move'];
        ...
        if (array_key_exists("sports_place",$formFields) and !is_null($formFields["sports_type"]) and !is_null($formFields["sports_time"])){
            if ($formFields["sports_place"] == "屋外"){
                ...
                $sports_carbon = ($sportsCarbonRate[$formFields["sports_type"]] ?? 0) *0.445 * $formFields["sports_time"]; 
            }
        }else{
            $sports_carbon = 0;
        }
    ...
    private function normaliseFormFields(Request $request): array
    {
        $formFields = $request->all();
        $formFields['user_id'] = auth()->id();

        foreach (['work_office', 'work_soho', 'work_3pl', 'life', 'move'] as $field) {
            $formFields[$field] = implode(',', Arr::wrap($request->input($field, [])));
        }

        foreach (['green_power', 'light_led_office', 'light_led_soho', 'light_led_3pl', 'move_walk'] as $field) {
            $formFields[$field] = $request->boolean($field) ? '1' : '0';
        }

        foreach ([
            'breakfast_image' => 'breakfast',
            'lunch_image' => 'lunch',
            'dinner_image' => 'dinner',
        ] as $field => $directory) {
            if ($request->hasFile($field)) {
                $formFields[$field] = $request->file($field)->store($directory, 'public');
            }
        }
    ...
    private function countSelections(?string $csv): int
    {
        if (empty($csv)) {
            return 0;
        }

        $segments = array_filter(array_map('trim', explode(',', $csv)), static function ($value) {
            return $value !== '';
        });
    ...
    private function rates(): array
    {
        ...
        $sportsRates = config('today.sports_carbon_rate', []);

        $cache = [
            'power_company' => config('today.power_company_rates', []),
            'food' => config('today.food_carbon_rate', []),
            'rice' => config('today.rice_carbon_rate', []),
            'vegetable_volume' => config('today.vegetable_volume_rate', []),
            'vegetable_carbon' => config('today.vegetable_carbon_rate', []),
            'main' => config('today.main_carbon_rate', []),
            'main_volume' => config('today.main_volume_rate', []),
            'sports_store' => $sportsRates['store'] ?? [],
            'sports_update' => $sportsRates['update'] ?? ($sportsRates['store'] ?? []),
            'rest' => config('today.rest_carbon_rate', []),
            'shopping' => config('today.shopping_carbon_rate', []),
            'move' => config('today.move_type_rate', []),
        ];
